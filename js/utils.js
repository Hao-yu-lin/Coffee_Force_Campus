 ...
 *
 * TXT files that have been merged with refractometer data (externally — the
 * app no longer does the merging) additionally carry per-second thermometer /
 * EC series, returned as top-level `thermometer` and `ec` (null when absent).
 *
...
function parseTxtBrewingLog(jsonText) {
    try {
        ...
        // Merged series may contain gaps — keep null so charts break the
        ...
        return {
            ...
            // ── Refractometer series (only present in merged TXT files) ────
            ...
            extra: {
                ...
                EC:                  log.EC,                 // conductivity per second
